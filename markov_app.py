...
def draw_bidirectional_transition(ax, x1, y1, x2, y2, prob1, prob2, cmap, norm):
    ...
    # forward path
    path1 = mpl.path.Path(
        [(start1_x, start1_y), (control1_x, control1_y), (end1_x, end1_y)],
    ...
    # backward path
    path2 = mpl.path.Path(
        [(end1_x, end1_y), (control2_x, control2_y), (start1_x, start1_y)],
        [mpl.path.Path.MOVETO, mpl.path.Path.CURVE3, mpl.path.Path.CURVE3]
    )
...
def main():
    ...
    if st.sidebar.button("Calculate"):
        ...
        for i, (x, y) in enumerate(positions):
            ...
            ax.add_patch(circle)
            ax.text(x, y, f"S{i+1}", ha='center', va='center', fontsize=14, fontweight='bold')
    ...
    st.markdown(
        ...
        <div style='text-align: center;'>
            <span class='rainbow-text'> ❤️ for Probability by Vivekanand Pawar</span>
        </div>
        """,
